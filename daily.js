...
Elm.DailyReview = Elm.DailyReview || {};
Elm.DailyReview.make = function (_elm) {
   "use strict";
   _elm.DailyReview = _elm.DailyReview || {};
   if (_elm.DailyReview.values) return _elm.DailyReview.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var generalStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                      ,_0: "font-size"
                                                      ,_1: "16px"}
                                                     ,{ctor: "_Tuple2",_0: "font-family",_1: "monospace"}
                                                     ,{ctor: "_Tuple2",_0: "width",_1: "700px"}]));
   var myUpdate = F2(function (action,model) {    return model;});
   var myView = F2(function (address,currentTime) {
      return A2($Html.div,
      _U.list([generalStyle]),
      _U.list([$Html.text($Basics.toString(currentTime))]));
   });
   var currentTime = Elm.Native.Port.make(_elm).inbound("currentTime",
   "Int",
   function (v) {
      return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",
      v);
   });
   var names = $Array.fromList(_U.list([]));
   var main = $StartApp$Simple.start({model: currentTime
                                     ,view: myView
                                     ,update: myUpdate});
   return _elm.DailyReview.values = {_op: _op
                                    ,main: main
                                    ,names: names
                                    ,myView: myView
                                    ,myUpdate: myUpdate
